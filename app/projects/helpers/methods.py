...
def check_date_format(dates):
    """Ensure that a date is correctly formatted.

    Ensures that start dates precede end dates.
...
def get_page(pages, page):
    """Get a page from a list of pages.

    Takes a list of paginated results from `paginate()` and
...
def delete_project(name, metapath):
    """Delete a project manifest based on name.
...
def search_collections(values):
    ...
    if list(corpus_db.find()):
        ...
        if values['regex'] is True:
            query = {}
            for k, v in values['query_properties'].items():
                REGEX = re.compile(v)
                query[k] = {'$regex': REGEX}
...
def textarea2dict(fieldname, textarea, main_key, valid_props):
    ...
    The key must be separated from the value by a space, and the main key should come
    first. If ": " occurs in the value, the entire value can be put in quotes.
...
def flatten_datelist(all_lines):
    ...
    Removes 'text' and 'format' properties and replaces
    their container dicts with a simple date string.
    """
    flattened = []
