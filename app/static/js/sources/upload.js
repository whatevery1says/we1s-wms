  ...
  $(document).ready(function () {
    ...
    var myDropzone = new Dropzone(document.body, {
      url: '/sources/import-sources',
      maxFiles: 1,//max number of files
      acceptedFiles: ".csv, .tsv,.xls, .xlt, .xlm, .xlsx, .xlsm, .xltx, .xltm, .xlsb, .xla, .xlam, .xll ,.xlw, .ods, .zip, .rar",//file types
    ...
    myDropzone.on('complete', function(response) {
      ...
      if (response['errors'].length > 0) {
        ...
        // Or show success
        msg = 'The source was imported successfully.'
      }
      bootbox.alert(msg)
